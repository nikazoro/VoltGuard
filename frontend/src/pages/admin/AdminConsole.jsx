...
import {
    ...
    Activity, MapPin, Users as UsersIcon, Zap, UserX, UserCheck
} from 'lucide-react';
...
const AdminConsole = () => {
    ...
    const updateStatusMutation = useMutation({
        mutationFn: ({ stationId, status }) =>
            adminAPI.updateStationStatus(stationId, status),
    ...
    const disableUserMutation = useMutation({
        ...
        onSuccess: (response) => {
            toast({
                title: 'User Disabled',
                description: `User ID ${response.data.user_id} has been disabled.`,
                variant: 'success',
            });
            setUserIdInput('');
            setConfirmAction(null);
    ...
    const enableUserMutation = useMutation({
        ...
        onSuccess: (response) => {
            toast({
                title: 'User Enabled',
                description: `User ID ${response.data.user_id} has been enabled.`,
                variant: 'success',
            });
            setUserIdInput('');
            setConfirmAction(null);
    ...
    const handleDisableUser = () => {
        ...
        setConfirmAction({
            type: 'disable_user',
            userId,
            action: () => disableUserMutation.mutate(userId),
        });
    };
    ...
    const handleEnableUser = () => {
        ...
        setConfirmAction({
            type: 'enable_user',
            userId,
            action: () => enableUserMutation.mutate(userId),
        });
    };
                            ...
                            <CardContent>
                                <div className="space-y-6">
                                    <div className="space-y-2">
                                        ...
                                        />
                                        <p className="text-xs text-muted-foreground">
                                            Enter the numeric ID of the user to manage
                                        </p>
                                    </div>
                                    ...
                                    <div className="flex gap-3">
                                        <Button
                                            ...
                                            disabled={!userIdInput || disableUserMutation.isPending}
                                        >
                                        ...
                                        <Button
                                            ...
                                            disabled={!userIdInput || enableUserMutation.isPending}
                                        >
                                    ...
                                    <div className="p-4 bg-muted rounded-lg">
                                        <h4 className="font-semibold text-sm mb-2">About User Management</h4>
                                        <ul className="text-xs text-muted-foreground space-y-1">
                                            <li>• Disabling a user prevents them from logging in</li>
                                            <li>• Enabling a user restores their access</li>
                                            <li>• Changes take effect immediately</li>
                                            <li>• Only admins can perform these actions</li>
                                        </ul>
                                    </div>
                                </div>
                            </CardContent>
                ...
                <DialogContent>
                    <DialogHeader>
                        ...
                        <DialogDescription>
                            ...
                            {confirmAction?.type === 'disable_user' && (
                                `Are you sure you want to disable User ID ${confirmAction?.userId}? They will not be able to log in.`
                            )}
                            {confirmAction?.type === 'enable_user' && (
                                `Are you sure you want to enable User ID ${confirmAction?.userId}? They will be able to log in again.`
                            )}
                        </DialogDescription>
                    </DialogHeader>
                    <DialogFooter>
                        <Button
                            ...
                            disabled={updateStatusMutation.isPending || disableUserMutation.isPending || enableUserMutation.isPending}
                        >
                            Cancel
                        </Button>
                        <Button
                            variant={
                                confirmAction?.newStatus === 'offline' || confirmAction?.type === 'disable_user'
                                    ? 'destructive'
                                    : 'default'
                            }
                            ...
                            disabled={updateStatusMutation.isPending || disableUserMutation.isPending || enableUserMutation.isPending}
                        >
                            {(updateStatusMutation.isPending || disableUserMutation.isPending || enableUserMutation.isPending)
                                ? 'Processing...'
                                : confirmAction?.type === 'status'
