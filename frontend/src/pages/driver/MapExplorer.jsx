...
const INDIAN_CITIES = [
    ...
    { name: 'Greater Noida', lat: 28.46524126826098, lng: 77.51117273171879 },
...
const RADIUS_OPTIONS = [5, 10, 20, 40, 50, 100, 1000];
...
const MapExplorer = () => {
    ...
    const [mapCenter, setMapCenter] = useState([28.46524126826098, 77.51117273171879]); // Greater Noida default
    const [mapZoom, setMapZoom] = useState(12);

    // Search filters
    const [searchLocation, setSearchLocation] = useState('Greater Noida');
    const [userLocation, setUserLocation] = useState({ lat: 28.46524126826098, lng: 77.51117273171879 });
    ...
    const { data: stationsData, isLoading, error, refetch } = useQuery({
        ...
        queryFn: async () => {
            console.log('[FRONTEND] Fetching stations with params:', {
                user_lat: userLocation.lat,
                user_lng: userLocation.lng,
                radius_km: radiusKm
            });
            const response = await stationAPI.getMapView({
                user_lat: userLocation.lat,
                user_lng: userLocation.lng,
                radius_km: radiusKm
            });
        ...
        return (
            <div className="h-[calc(100vh-4rem)] flex items-center justify-center">
                <Card className="max-w-md mx-auto p-8 text-center">
                    ...
                    <h3 className="text-lg font-semibold mb-2">Unable to Load Stations</h3>
                    <p className="text-muted-foreground mb-4">
                        {error.response?.data?.detail || error.message || 'Failed to fetch station data. Please try again.'}
                    </p>
                    ...
                    <div className="p-4 space-y-3">
                        ...
                        {stations.length === 0 && (
                            <div className="text-center py-12 text-muted-foreground">
                                ...
                                <p className="font-medium">No stations found within {radiusKm} km</p>
                                <p className="text-xs mt-2">Try increasing the search radius or changing location</p>
                                <Button
                                    variant="outline"
                                    size="sm"
                                    className="mt-4"
                ...
                <div className="flex-1 relative">
                    ...
                    <div className="absolute top-4 left-4 bg-background/95 backdrop-blur rounded-lg shadow-lg p-3 z-[1000]">
                        <div className="flex items-center gap-2 text-sm">
                            <MapPin className="h-4 w-4 text-slate-900 dark:text-slate-100" />
                            <div>
