...
@router.get(
    "/stations/map",
    dependencies=[Depends(require_role(UserRole.driver, UserRole.station_owner, UserRole.admin))],
)
async def driver_station_map(
    ...
    print(f"[DEBUG] Driver map request: lat={user_lat}, lng={user_lng}, radius={radius_km}km")
    now = datetime.utcnow()
    ...
    print(f"[DEBUG] Found {len(rows)} stations within {radius_km}km radius")
    ...
    print(f"[DEBUG] Returning {len(response)} stations to client")
    return response
